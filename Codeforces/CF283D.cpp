...
void solve(){
    ...
    REP_1(i, N){
        ...
        dp[i] = i-1;//x 1 1 1 1 ... current=1
        LL y = seq[i];
        for(int j = i-1; j >= 1; j--){
            LL x = seq[j];
            if(y&1LL){
                // x y y y y ...
                if(x%y == 0) dp[i] = min(dp[i],dp[j]+i-j-1);
            }else{
                //x ... y/16 y/8 y/4 y/2 y
                if(x%y == y/2LL) dp[i] = min(dp[i],dp[j]+i-j-1);
                y >>= 1LL;
            }
        }
        ret = min(ret,dp[i]+((LL)N-(LL)i));//Fill 1 afterward
    }
    cout << ret << endl;
}
