...
LL buildsg(LL a,LL x,LL k){
    LL att = x+(x+k-2LL)/(k-1LL);
    while(att < (a+k-1LL)/k){
        att = att*k+1LL;
    }
    return att;
...
int main(int argc, const char * argv[]){
    ...
    while(scanf(" %lld %lld",&N,&K) != EOF){
        REP_1(i, N) scanf(" %lld",&A[i]);
        LL tar = 0;
        REP_1(i, N) tar ^= getsg(A[i],K);
        if(!tar) puts("Bob");
        else {
            REP_1(i, N){
                if(A[i] != 1 && buildsg(A[i],tar^getsg(A[i],K), K) < A[i]){
                printf("Alice %d %lld\n",i,buildsg(A[i],tar^getsg(A[i],K), K));
                    break;
                }
            }
